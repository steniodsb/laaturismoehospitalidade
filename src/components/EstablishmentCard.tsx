...
import { categoryLabels, categoryIcons, type Establishment } from "@/data/mockData";
    ...
    >
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <span className="inline-flex items-center gap-1 bg-card/90 backdrop-blur-sm text-foreground text-xs font-medium px-2.5 py-1 rounded-full">
            {categoryIcons[establishment.category]} {categoryLabels[establishment.category]}
          </span>
        </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-1 mb-2">
          ...
          <span className="text-xs text-muted-foreground">{establishment.cityName}</span>
        </div>
        ...
        <div className="flex flex-wrap gap-1 mb-3">
          {establishment.amenities.slice(0, 3).map((amenity) => (
            <span key={amenity} className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-full">
              {amenity}
            </span>
          ))}
        </div>
