...
const EstablishmentDetailPage = () => {
  ...
  useEffect(() => {
    ...
    supabase
      .from("establishments")
      .select("id, name, image_url, short_description, description, amenities, rating, phone, whatsapp, address, latitude, longitude, details, category:categories(name, icon, slug), city:cities(name)")
      .eq("id", id)
      .single()
      .then(({ data }) => {
        if (data) setEst(data as unknown as EstFull);
        setLoading(false);
      });
  }, [id]);
  ...
  const d = (est.details || {}) as Record<string, any>;
  const categorySlug = est.category?.slug || "";
  const isHospedagem = categorySlug === "hospedagem";
  const isGastronomia = categorySlug === "gastronomia";
  const gallery: { url: string; caption?: string }[] = d.gallery || [];
  const allImages = gallery.length > 0 ? gallery : [{ url: est.image_url || "/placeholder.svg", caption: est.name }];
  const amenities = est.amenities || [];
  ...
  const openRoute = () => {
    if (est.latitude && est.longitude) {
      ...
      window.open(`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(est.address)}`, "_blank");
    }
  };

  const openMap = () => {
    if (est.latitude && est.longitude) {
      window.open(`https://www.google.com/maps?q=${est.latitude},${est.longitude}`, "_blank");
    }
  };
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative h-[45vh] min-h-[280px] max-h-[450px] overflow-hidden">
        <img src={est.image_url || "/placeholder.svg"} alt={est.name} className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-foreground/80 via-foreground/30 to-transparent" />
        <div className="absolute bottom-0 left-0 right-0">
          <div className="container pb-8">
            <Link to="/explorar" className="inline-flex items-center gap-1 text-sm text-background/70 hover:text-background mb-3 transition-colors">
            ...
            {est.category && (
              <div className="flex items-center gap-2 mb-2">
                <span className="inline-flex items-center gap-1 bg-primary/90 text-primary-foreground text-xs font-medium px-2.5 py-1 rounded-full">
                  {est.category.icon} {est.category.name}
                </span>
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center gap-4 text-background/70 text-sm">
              ...
              {isHospedagem && d.priceRange && <span className="text-background/90 font-medium">{d.priceRange}</span>}
              {isGastronomia && d.cuisineType && <span className="text-background/90 font-medium">{d.cuisineType}</span>}
            </div>
          </div>
        </div>
      </section>
        ...
        <section className="bg-muted/50 border-b border-border">
          <div className="container py-4">
            <div className="flex gap-3 overflow-x-auto pb-2">
              ...
              <button onClick={() => { setGalleryIndex(0); setGalleryOpen(true); }} className="flex-shrink-0 w-28 h-20 rounded-lg border-2 border-dashed border-border bg-muted flex items-center justify-center text-sm text-muted-foreground hover:border-primary hover:text-primary transition-colors">Ver todas</button>
            </div>
          </div>
        </section>
      )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              ...
              {isHospedagem && (
                <div className="bg-muted/30 rounded-xl border border-border p-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    ...
                    {d.totalRooms && <DetailItem icon={<Bed className="h-5 w-5 text-primary mt-0.5" />} label="Quartos" value={`${d.totalRooms} unidades`} />}
                    {d.breakfastIncluded !== undefined && <DetailItem icon={<Coffee className="h-5 w-5 text-primary mt-0.5" />} label="Café da Manhã" value={d.breakfastIncluded ? "Incluso" : "Não incluso"} />}
                    {d.parkingIncluded !== undefined && <DetailItem icon={<Car className="h-5 w-5 text-primary mt-0.5" />} label="Estacionamento" value={d.parkingIncluded ? "Gratuito" : "Pago"} />}
                    {d.petsAllowed !== undefined && <DetailItem icon={<DogIcon className="h-5 w-5 text-primary mt-0.5" />} label="Pets" value={d.petsAllowed ? "Aceitos" : "Não aceitos"} />}
                    {d.languages?.length > 0 && <DetailItem icon={<Globe className="h-5 w-5 text-primary mt-0.5" />} label="Idiomas" value={d.languages.join(", ")} />}
                  </div>
                  {d.roomTypes?.length > 0 && (
                    <div className="mt-5 pt-5 border-t border-border">
                      <p className="text-sm font-semibold text-foreground mb-2">Tipos de Quarto</p>
                      <div className="flex flex-wrap gap-2">
                        {d.roomTypes.map((rt: string) => (
                          <span key={rt} className="inline-flex items-center gap-1.5 bg-primary/10 text-primary text-sm px-3 py-1.5 rounded-full font-medium"><Bed className="h-3.5 w-3.5" /> {rt}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  {d.cancellationPolicy && (
                    <div className="mt-4 pt-4 border-t border-border">
                      <p className="text-sm text-muted-foreground"><span className="font-semibold text-foreground">Cancelamento:</span> {d.cancellationPolicy}</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {isGastronomia && (
                <div className="bg-muted/30 rounded-xl border border-border p-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    ...
                    {d.capacity && <DetailItem icon={<Users className="h-5 w-5 text-primary mt-0.5" />} label="Capacidade" value={`${d.capacity} pessoas`} />}
                    {d.chefName && <DetailItem icon={<ChefHat className="h-5 w-5 text-primary mt-0.5" />} label="Chef" value={d.chefName} />}
                    {d.reservationRequired !== undefined && <DetailItem icon={<CalendarCheck className="h-5 w-5 text-primary mt-0.5" />} label="Reserva" value={d.reservationRequired ? "Recomendada" : "Não necessária"} />}
                    {d.delivery !== undefined && <DetailItem icon={<Truck className="h-5 w-5 text-primary mt-0.5" />} label="Delivery" value={d.delivery ? "Disponível" : "Não disponível"} />}
                    {d.liveMusic !== undefined && <DetailItem icon={<Music className="h-5 w-5 text-primary mt-0.5" />} label="Música ao Vivo" value={d.liveMusic ? "Sim" : "Não"} />}
                    {d.outdoorSeating !== undefined && <DetailItem icon={<TreePine className="h-5 w-5 text-primary mt-0.5" />} label="Área Externa" value={d.outdoorSeating ? "Disponível" : "Não disponível"} />}
                  </div>
                    ...
                    <div className="mt-5 pt-5 border-t border-border">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {d.specialties.map((s: string) => (
                          <span key={s} className="inline-flex items-center gap-1.5 bg-secondary/10 text-secondary text-sm px-3 py-1.5 rounded-full font-medium"><UtensilsCrossed className="h-3.5 w-3.5" /> {s}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  {d.paymentMethods?.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-border">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {d.paymentMethods.map((p: string) => (
                          <span key={p} className="inline-flex items-center gap-1.5 bg-muted text-foreground text-xs px-2.5 py-1 rounded-full"><CreditCard className="h-3 w-3" /> {p}</span>
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {(est.latitude && est.longitude) && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-3 mt-4">
                    <Button onClick={openRoute} className="gap-2"><Navigation className="h-4 w-4" /> Iniciar Rota</Button>
                    <Button variant="outline" onClick={openMap} className="gap-2"><MapPin className="h-4 w-4" /> Abrir no Google Maps</Button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-6">
              <div className="bg-card rounded-xl border border-border p-6 shadow-card">
                ...
                {!est.address && cityName && (
                  <div className="flex items-center gap-3 mb-3 text-sm">
                    <MapPin className="h-4 w-4 text-primary" />
                    <span className="text-muted-foreground">{cityName}, SP</span>
                  </div>
                )}
                {est.phone && (
                  <div className="flex items-center gap-3 mb-3 text-sm">
                    <Phone className="h-4 w-4 text-primary" />
                    <a href={`tel:${est.phone.replace(/\D/g, "")}`} className="text-muted-foreground hover:text-primary transition-colors">{est.phone}</a>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  {est.whatsapp && (
                    <a href={`https://wa.me/${est.whatsapp}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full bg-[#25D366] text-white font-semibold py-3 rounded-lg hover:bg-[#20bd5a] transition-colors">
                      <MessageCircle className="h-4 w-4" /> WhatsApp
                    </a>
                  )}
                  <a href={est.phone ? `tel:${est.phone.replace(/\D/g, "")}` : "#"} className="block w-full text-center bg-primary text-primary-foreground font-semibold py-3 rounded-lg hover:bg-primary/90 transition-colors">Ligar Agora</a>
                </div>
              </div>

              {isHospedagem && d.priceRange && (
                <div className="bg-primary/5 rounded-xl border border-primary/20 p-6">
                  <p className="text-sm text-muted-foreground mb-1">A partir de</p>
                  <p className="text-2xl font-bold text-primary">{d.priceRange}</p>
                  <p className="text-xs text-muted-foreground mt-1">por noite</p>
                </div>
              )}

              {isGastronomia && d.priceRange && (
                <div className="bg-secondary/5 rounded-xl border border-secondary/20 p-6">
                  ...
                  <p className="text-2xl font-bold text-secondary">{d.priceRange}</p>
